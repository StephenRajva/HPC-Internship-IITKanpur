#!/usr/bin/env python
# =============================================================================
#  minimd_dfly_ppn1.py  -  MiniMD on a Dragonfly, PPN (ranks per node) = 1
# =============================================================================
#  Dragonfly nodes = hosts_per_router * routers_per_group * num_groups.
#  We FIX hosts_per_router = 1 (so each router holds one node) and grow the
#  group structure to reach the target size:
#
...
#       64          8               8        64
#
#  job["size"] = nodes  ->  PPN = 1.
#
#  USAGE:  export DFLY_SIZE="16"   (then) sst minimd_dfly_ppn1.py
...
# Router config (matches your MiniMD mesh/torus settings)
router                 = hr_router()
...
# MiniMD motifs (same as your mesh/torus MiniMD runs -> fair comparison)
motifs = [
...
csv_path = os.path.join(out_dir, f"minimd_dragonfly_{size_label}_ppn1_nodes{num_nodes}_stats.csv")
