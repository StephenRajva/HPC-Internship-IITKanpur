#!/usr/bin/env python
# =============================================================================
#  minimd_mesh3d_ppn2.py  -  MiniMD on a 3D mesh, PPN (ranks per node) = 2
# =============================================================================
#  Same study as minimd_mesh3d_ppn1.py but 2 hosts per router -> PPN = 2.
#  Router grid (shape) stays the same as PPN=1; nodes & ranks double.
#  PPN = ranks / routers = (2 * routers) / routers = 2.
#
#  USAGE:  export MESH_SHAPE="4x4x4"   (then) sst minimd_mesh3d_ppn2.py
#  The runner script loops over all shapes.
# =============================================================================
...
for d in dims:
    num_routers *= d
num_nodes = num_routers * nodes_per_router        # 2 hosts per router
num_ranks = num_nodes                             # PPN = 2 (ranks == nodes)
...
# ---- MiniMD workload (identical motifs to PPN=1) ----------------------------
motifs = [
...
# ---- statistics -> one CSV per shape (filename marks ppn2) ------------------
...
# ---- summary print ----------------------------------------------------------
print("==================================================")
print(f" Topology          : 3D Mesh {shape} - MiniMD")
