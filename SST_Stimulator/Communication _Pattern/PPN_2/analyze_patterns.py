...
# (label, shape, nodes)
SIZES = [("2x2", 8), ("4x4", 32), ("8x8", 128), ("16x16", 512), ("32x32", 2048)]
...
def hops(pattern, shape, N):
    ...
    a, b = coord(0 // 2, dims), coord(p // 2, dims)   # PPN=2: node id // 2 = router id
...
for p in PATTERNS:
    for shape, N in SIZES:
        csv_path = os.path.join(p, "results", f"{p}_mesh_{shape}_ppn2_nodes{N}_stats.csv")
...
plt.xlabel("Nodes (PPN=2)"); plt.ylabel("Avg packet latency (ns)")
plt.title("Communication patterns on Mesh - Latency vs Size (PPN=2)")
plt.legend(); plt.grid(True, which="both", ls=":")
plt.savefig("pattern_latency_vs_size_ppn2.png", dpi=130, bbox_inches="tight")
print("Saved: pattern_latency_vs_size_ppn2.png")
...
plt.xscale("log", base=2)
plt.xlabel("Nodes (PPN=2)"); plt.ylabel("Representative hops (Manhattan)")
plt.title("Communication patterns on Mesh - Hops vs Size (PPN=2)")
plt.legend(); plt.grid(True, which="both", ls=":")
plt.savefig("pattern_hops_vs_size_ppn2.png", dpi=130, bbox_inches="tight")
print("Saved: pattern_hops_vs_size_ppn2.png")
